...
design<-svydesign(id=~psu, strata=~kstrata,weights=~wt_tot,data=data4,nest=TRUE)
...
write.csv(Mat_final,"Max&Min_p_val_&coef_final_statistics_with_KNHANES_AllF-Nutrient-svyglm.csv")

### And To obtain results for FPro - only:

rownames(Matfull2)<-colnames(Mat1)
write.csv(Matfull2,"FPro_p_val_&coef_final_statistics_with_KNHANES_AllF-Nutrient-svyglm.csv")


### F15-49 Survey statistics ####
...
colnames(Mat_final)<-colnames(Mat)
rownames(Mat_final)<-c("max_p","min_p","Coef(max_map)","Coef(min_p)","Nut_top_conf")
write.csv(Mat_final,"Max&Min_p_val_&coef_final_statistics_with_KNHANES_F15-49-Nutrient-svyglm.csv")

### And To obtain results for FPro - only:

write.csv(Matfull2,"FPro_p_val_&coef_final_statistics_with_KNHANES_F15-49-Nutrient-svyglm.csv")
