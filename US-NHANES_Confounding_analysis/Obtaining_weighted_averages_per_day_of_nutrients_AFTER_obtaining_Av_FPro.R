...
Sums_Food<-matrix(rep(0,nrow(A)*62),nrow=nrow(A))
Aver_Food<-matrix(rep(0,nrow(A)*62),nrow=nrow(A))
...
Sums_Food[i,]<-Sums_Food[i,]+as.vector(t((F_code_consumption_val[j])* W[match(F_code[j],W[,ncol(W)]),1:62]))
k<-k+(F_code_consumption_val[j])}
Aver_Food[i,]<-Sums_Food[i,]/k}

if (any(is.na(Aver_Food))==TRUE) {
Aver_Food[which(is.na(Aver_Food))]<-mean(Aver_Food[-which(is.na(Aver_Food))])}
...
At<-At[,-1]
colnames(Aver_Food)<-colnames(W[,1:62])
At2<-cbind(At,Aver_Food)

write.csv(At2,"Female_aged_15-49_with_NEG_UrTest_8907_with_Av_FPro_and_weighted_average_per_day_of_62_consumed_nutrients_under_1888_removed-codes-terms.csv")  ####### MARK for Confounding_analysis_Setting_input_datasets_code.R
...
write.csv(At2,"Female_aged_50_and_above_with_Av_FPro_and_weighted_average_per_day_of_62_consumed_nutrients_under_1888_removed-codes-terms.csv")  ####### MARK for Confounding_analysis_Setting_input_datasets_code.R
